...
import { readFileSync, readdirSync, lstatSync, readFile } from "node:fs";
...
export class ServerShell {
   ...
   */
  use(
    middleware: (req: http.IncomingMessage, info: http.ServerResponse) => void
  ) {
  ...
  useDynamic(directory: string, route: string) {
    ...
    for (const entry of contents) {
      const pathnameDir = join(directory, entry);
      let pathnameRoute = join(route, entry);
      const dirEntry = lstatSync(pathnameDir);
      if (dirEntry.isDirectory()) {
        ...
        this.get(pathnameRoute.replaceAll("\\", "/"), (req, res) => {
          const assetContents = new TextDecoder("utf-8").decode(
            readFileSync(pathnameDir)
          );
          res.writeHead(200, {
            "Content-Type": MIMEFromExt(extension),
...
function MIMEFromExt(extension: string): string {
  switch (extension) {
    ...
    default: {
      console.log(
        `Encountered unknown extension while generating static routes: .${extension} - If you want this fixed as quickly as possible, open an issue at https://github.com/SaphirDeFeu/TSMServerShell-Node/issues`
      );
      return "text/plain";
    }
  }
